...
2. Run local validation before pushing. Prefer repo instructions in
   `AGENTS.md`, then `make validate` when present, and always run
   `git diff --check`.
...
6. Write/update PR body explicitly. If `.github/pull_request_template.md`
   exists, use it and fill every section with concrete content for this
   change.
